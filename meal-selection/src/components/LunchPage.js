...
const LunchPage = ({ addItem, searchQuery, itemCounts, removeItem }) => {
  const [customFoodItems, setCustomFoodItems] = useState(() => {
    const storedItems = localStorage.getItem('customLunchItems');
  ...
  const handleAddFood = (e) => {
    ...
    if (newFoodName && newFoodImage) {
      ...
      setShowForm(false);
      localStorage.setItem('customLunchItems', JSON.stringify(updatedItems));
    } else {
